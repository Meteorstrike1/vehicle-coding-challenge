...
# This rake task is extremely slow and needs optimising
desc 'Load vehicle data and attempt to generate vehicle registration number without duplicates'
task :generate_vrns_removing_duplicates do
  filename = 'vehicles.csv'
  ...
  imported_csv.each do |vehicle|
    ...
    if successful_vrns.include?(vrn)
      fail_count += 1
      LOG.debug { 'Failed to generate VRN: VRN already exists' }
    else
      successful_vrns.append(vrn)
      pass_count += 1
    end
